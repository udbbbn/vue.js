...
export default {
  ...
  props: ['allList', 'userList', 'showIndex', 'userFlag'],
  components: {
    news
  },
  methods: {
    ...
    sendAjax: function(sendSetting) {
      this.$ajax({
        method: 'post',
        url: 'http://localhost:8033/send',
        data: {
          sendSetting: JSON.stringify(sendSetting)
        }
      })
        .then((res) => {
          console.log(res)
